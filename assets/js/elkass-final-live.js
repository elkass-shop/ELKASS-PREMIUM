...
function productCard(p){ return `<a class="ef-product" href="app/product/?id=${encodeURIComponent(p.id)}"><div class="ef-product-media"><span class="ef-product-badge">${displayBadge(p)}</span><img src="${p.image||imgFallback}" alt="${p.name}" loading="lazy"></div><div class="ef-product-body"><small>${p.brand||''} • ${p.category||''}</small><h3>${p.name}</h3><p>${p.short||''}</p><div class="row"><strong>${fmt(p.price)}</strong><span class="view">Zobacz</span></div></div></a>` }
